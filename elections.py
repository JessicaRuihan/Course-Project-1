...
class Election:
    ...
    def read_results(self, input_stream: TextIO) -> None:
        ...
        for lst in results:
            ...
            self.update_results(riding, party, int(lst[17].replace('\"', '',
                                                                   2)))
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        ['pc']
        """
        party_votes = self._results[riding]
        votes_party = {}
        for i in party_votes.keys():
            if party_votes[i] not in votes_party:
                votes_party[party_votes[i]] = []
            votes_party[party_votes[i]].append(i)
        return votes_party[max(votes_party.keys())]
    ...
    def popular_vote(self) -> Dict[str, int]:
        ...
        """
        popular_vote = {}
        for r in self._ridings:
            for p in self._results[r]:
                if p not in popular_vote:
                    popular_vote[p] = 0
                popular_vote[p] += self._results[r][p]
        return popular_vote
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        True
        """
        party_num_ridings_won = {}
        for p in self._parties:
            party_num_ridings_won[p] = 0
        for r in self._ridings:
            winners = self.riding_winners(r)
            if len(winners) == 1:
                party_num_ridings_won[winners[0]] += 1
        return party_num_ridings_won
    ...
    def election_winners(self) -> List[str]:
        ...
        ['pc']
        """
        party_num_ridings_won = self.party_seats()
        most_seats = 0
        party_most_seats = []
        for p in party_num_ridings_won:
            if party_num_ridings_won[p] > most_seats:
                party_most_seats.clear()
                party_most_seats.append(p)
                most_seats = party_num_ridings_won[p]
            elif party_num_ridings_won[p] == most_seats:
                party_most_seats.append(p)
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int,
        ...
        if date_today not in self._elections:
            self._elections[date_today] = Election(date_today)
        self._elections[date_today].read_results(input_stream)
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for e in self._elections:
            p = self._elections[e].election_winners()
            if party in p:
                dates.append(e)
        return dates
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        party_percentage = {}
        for e in self._elections:
            popular_vote = self._elections[e].popular_vote()
            if party not in popular_vote:
                party_percentage[e] = 0
            else:
                values = popular_vote.values()
                party_percentage[e] = round(popular_vote[party]
                                            / sum(values), 2)
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        True
        """
        dates = list(self._elections.keys())
        dates.sort()
        changes_in_ridings = []
        for i in range(len(dates) - 1):
            first_election = set(self._elections[dates[i]].ridings_recorded())
            second_election = set(self._elections[dates[i + 1]].ridings_recorded
                                  ())
            first_and_second = first_election.intersection(second_election)
            first_only = first_election - first_and_second
            second_only = second_election - first_and_second
            changes_in_ridings.append((first_only, second_only))
        return changes_in_ridings
